...
This is the Phase 0 inventory for the current `omicsprism-platform` tree. It
describes runtime ownership before the v3 extraction work; it is not a runtime
registry and should be updated only when a refactor changes the ownership map.
...
| --- | ---: | --- | --- | --- |
| `__init__.py` | 72 | Package exports for stores, model, router, runtime and tools | `backend.agent_worker`, tests, API bootstrap | `extract`; remove legacy exports in Phase 5 |
...
| `runtime.py` | 1649 | `ProductionRunCoordinator`, fixture coordinator, contrast completion, orchestration helpers | worker, API bootstrap, eval, end-to-end tests | `extract` resolver/validation; `delete in Phase 5` coordinator |
